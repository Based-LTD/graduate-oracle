...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        if (err instanceof z.ZodError) {
            throw new McpError(ErrorCode.InvalidParams, `Invalid arguments for ${name}: ${err.errors.map(e => `${e.path.join(".")} - ${e.message}`).join("; ")}`);
        }
        throw new McpError(ErrorCode.InternalError, `${name} failed: ${err.message}`);
    }
});
async function main() {
