...
export const accuracy_receipts = {
    ...
    description: "Get the live, calibrated accuracy receipts for graduate-oracle. Returns " +
    ...
    async handler() {
        try {
            const d = await api.get("/api/accuracy", { withAuth: false });
            const h = (d.headline ?? {});
            const last30 = (h.last_30d ?? {});
            const timing = (h.time_to_grad ?? {});
            const tiers = (h.tier_runways ?? {});
            const traj = (h.trajectory ?? []);
            const deployedAt = h.model_deployed_at;
            const deployedIso = typeof deployedAt === "number"
                ? new Date(deployedAt * 1000).toISOString().slice(0, 10)
                : null;
            const recall = (h.recall_30d ?? {});
            const runnerRates = (h.post_grad_runner_rates ?? {});
            const fmtRunner = (cell) => {
            ...
            const msg = err instanceof ApiError
                ...
                : `accuracy_receipts failed: ${err.message}`;
