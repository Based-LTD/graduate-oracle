...
export const recent_graduations = {
    ...
    description: "POST-GRADUATION SCREEN. Lists pump.fun mints that just completed the bonding " +
    ...
    async handler(args) {
        ...
        try {
            ...
            const signals = await api.get(`/api/v1/signals?limit=${opts.limit * 4}`);
            const all = (signals.items ?? []);
            const recent = all
            ...
            const msg = err instanceof ApiError
                ...
                : `recent_graduations failed: ${err.message}`;
