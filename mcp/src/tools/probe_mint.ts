...
export const probe_mint = {
  name: "probe_mint",
  description:
    "Score a single pump.fun mint by address — works for BOTH live (pre-grad) AND " +
  ...
  async handler(args: unknown): Promise<{ content: Array<{ type: "text"; text: string }> }> {
    ...
    try {
      const d = await api.get(`/api/v1/probe/${encodeURIComponent(mint)}`) as Record<string, unknown>;
      ...
      const m = (typeof d.mint === "object" && d.mint !== null ? d.mint : d) as Record<string, unknown>;
      const summary = {
        mint: m.mint ?? mint,
      ...
      const msg = err instanceof ApiError
        ...
        : `probe_mint failed: ${(err as Error).message}`;
