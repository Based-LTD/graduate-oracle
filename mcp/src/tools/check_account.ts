...
export const check_account = {
  name: "check_account",
  description:
    "Get the current API account state — tier (free/tg_paid/builder/pro), " +
  ...
  async handler(): Promise<{ content: Array<{ type: "text"; text: string }> }> {
    ...
    try {
      const d = await api.get("/api/me") as Record<string, unknown>;
      const expiresAt = d.expires_at as number | null | undefined;
      ...
      const tierLimits = (d.tier_limits ?? {}) as Record<string, unknown>;
      ...
      const msg = err instanceof ApiError
        ...
        : `check_account failed: ${(err as Error).message}`;
