...
export const verify_prediction = {
  ...
  description:
    "Verify a graduate-oracle prediction against the on-chain receipts chain. " +
    ...
    try {
      if (opts.mint) {
        const d = await api.get(
          `/api/predictions/by_mint/${encodeURIComponent(opts.mint)}`,
          { withAuth: false },
      ...
      const d = await api.get(
        `/api/ledger/proof/${encodeURIComponent(String(opts.prediction_id))}`,
        { withAuth: false },
      );
      return {
        content: [{
      ...
      const msg = err instanceof ApiError
        ...
        : `verify_prediction failed: ${(err as Error).message}`;
